...
//
// Organizers can register from any country, so money shown in emails and API
// responses must follow the organizer's country. This is the backend mirror of
// `frontend/src/data/currencies.ts` — keep the two in sync when editing.
...
// code (e.g. a pending-subscription doc's `currency`) rather than a country.
...
/** Normalize any stored country form (ISO-2, name, ISO-3) to ISO-2, or "". */
...
/**
 * Format a money amount for a country: locale-grouped digits with the explicit
 * symbol prepended (e.g. "₹18,500.00", "SG$1,250.00", "€99.00"). We prepend our
 * own symbol rather than style:"currency" so distinct dollars don't collapse to
 * a bare "$".
 */
export function formatMoney(
  ...
  const cfg = currencyForCountry(country);
