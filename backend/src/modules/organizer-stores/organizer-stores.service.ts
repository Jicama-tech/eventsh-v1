...
export class OrganizerStoresService {
  ...
  async findBySlug(slug: string) {
    try {
      ...
      // If not found by slug, search stores that have a storeName matching the slug pattern
      const allStores = await this.organizerStoreModel
        .find({ "settings.general.storeName": { $exists: true } })
        .select("_id slug settings.general.storeName")
        .lean()
        .exec();
        ...
        try {
          ...
          if (genSlug === slug) {
            ...
            await this.organizerStoreModel
              .updateOne({ _id: s._id }, { $set: { slug: genSlug } })
              .exec();
            // Return the full document now
            return await this.organizerStoreModel.findById(s._id).exec();
          }
        } catch (genSlugError) {
