...
💰 *Total Amount:* ${formatCurrency(ticket.totalAmount || 0, country)}
...
Thank you for choosing Eventsh! 🎊`;
  ...
  private async updateEventTicketCount(
    eventId: string,
    quantity: number,
    ticketDetails?: { ticketType: string; quantity: number }[],
  ) {
          ...
          if (vt.maxCount !== undefined && vt.maxCount !== null) {
            if (vt.maxCount < detail.quantity) {
              throw new BadRequestException(
                `Not enough ${detail.ticketType} tickets available (${vt.maxCount} left)`,
              );
            }
            vt.maxCount -= detail.quantity;
          }
        }
      }
