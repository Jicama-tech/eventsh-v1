...
export function InlineWalkinForm({
  ...
  if (step === "done" && confirmation) {
    return wrap(
      <div className="bg-card border border-emerald-200 dark:border-emerald-800 rounded-md p-3 text-center">
        ...
        <div className="text-[11px] text-foreground text-left bg-muted rounded p-2 space-y-0.5">
          <div>
  ...
  if (step === "qr_payment") {
    return wrap(
      <div className="bg-card border rounded-md p-3 text-center">
        <div className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-primary/10 text-primary text-[9px] font-bold uppercase mb-1.5">
  ...
  if (step === "customer" && event) {
    return wrap(
      <div className="space-y-2.5">
        ...
        <div className="border rounded-md p-2 bg-card flex items-center justify-between text-xs">
          <div className="truncate">
            <span className="font-semibold text-foreground">{event.title}</span>
            <span className="text-muted-foreground"> · {effectiveType.name}</span>
          </div>
          <span
            className={`font-semibold ${totalPrice > 0 ? "text-foreground" : "text-success"}`}
          >
        ...
        {totalPrice > 0 && (
          <div>
            ...
            <div className="grid grid-cols-2 gap-1.5">
              ...
              >
                <Banknote
                  className={`h-3.5 w-3.5 ${paymentMethod === "cash" ? "text-success" : "text-muted-foreground"}`}
                />
              ...
              >
                <QrCode
                  className={`h-3.5 w-3.5 ${paymentMethod === "qr" ? "text-primary" : "text-muted-foreground"}`}
                />
        ...
        {error && (
          <p className="text-[11px] text-destructive bg-destructive/10 border border-destructive/30 rounded p-1.5">
            {error}
          </p>
        )}
        ...
        <div className="grid grid-cols-2 gap-1.5">
          {event.visitorTypes.map((t) => (
            <button
              ...
              className="border rounded p-1.5 text-left bg-card hover:border-primary hover:shadow-sm transition"
            >
              <div className="flex items-center justify-between gap-1">
                ...
                <span
                  className={`text-[11px] font-bold ${t.price > 0 ? "text-foreground" : "text-success"}`}
                >
        ...
        {payload.events.map((ev) => {
          ...
          return (
            <button
              ...
              className="w-full border rounded p-1.5 text-left bg-card hover:border-primary hover:shadow-sm transition flex items-center justify-between gap-2"
            >
              ...
              <div className="flex items-center gap-1.5 shrink-0">
                <span
                  className={`text-[11px] font-bold ${types.length === 0 ? "text-success" : "text-foreground"}`}
                >
                  {types.length === 0
                    ? "FREE"
