...
import { OrganizerStorefrontCustomizer } from "@/components/organizer/organizerStorefrontCustomizer";
...
export function EventfrontTemplate({ onBack }: { onBack: () => void }) {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        const [settingsRes, eventsRes, organizerRes, happAttendees] =
          await Promise.all([
            fetch(`${apiURL}/organizer-stores/organizer-store-detail`, {
              method: "GET",
              headers: { Authorization: `Bearer ${token}` },
            }),
            fetch(`${apiURL}/events/organizer/${organizerId}`),
            fetch(`${apiURL}/organizers/profile-get/${organizerId}`),
            fetch(`${apiURL}/tickets/organizer/${organizerId}`),
          ]);

        if (!settingsRes.ok || !eventsRes.ok || !organizerRes.ok) {
        ...
        const attendees = await happAttendees.json();

        setSettings(settingsData.data || settingsData);
        setOrganizerInfo(organizerData.data);
        setOrganizationName(organizerData.data.organizationName);
        setWhatsappNumber(organizerData.data?.whatsAppNumber || "");
        setAttendees(attendees.length);
  ...
  const handleEventClick = (eventId: string) => {
    navigate(`/event/${organizationName}/events/${eventId}`);
  };

  // Get random events for carousel
  ...
  const onShare = async () => {
    const shareUrl = `https://eventsh.com/organizers/${organizationName}`;
    if (navigator.share) {
      try {
      ...
      try {
        ...
        alert("Event organizer link copied to clipboard: " + shareUrl);
      } catch {
      ...
      <div className="bg-white dark:bg-gray-900 shadow-sm border-b p-3 sm:p-4 sticky top-0 z-50">
        <div className="flex items-center justify-between max-w-7xl mx-auto px-2 sm:px-4">
          ...
          {/* <Button
            ...
            className="bg-primary text-white text-xs sm:text-sm px-2 sm:px-4 py-1 sm:py-2"
            size="sm"
          >
